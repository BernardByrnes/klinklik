...
SYNTHETIC_ABDOMINAL = "Phase 1G verification - synthetic abdominal examination"
SYNTHETIC_ABDOMINAL_UPDATED = "Phase 1G verification - updated synthetic abdominal examination"
SYNTHETIC_ABDOMINAL_SIGNED = "Phase 1G verification - signed synthetic abdominal examination"
SYNTHETIC_NEUROLOGICAL = "Phase 1G verification - synthetic neurological examination"
SYNTHETIC_NEUROLOGICAL_UPDATED = "Phase 1G verification - updated synthetic neurological examination"
SYNTHETIC_NEUROLOGICAL_SIGNED = "Phase 1G verification - signed synthetic neurological examination"
SYNTHETIC_GENERAL = "Phase 1G verification - synthetic general examination"


def create_encounter(tenant, client, label="Phase1G"):
    patient = client.post(
        "/api/v1/patients/",
    ...
    triage = client.post(
        ...
        {"acuity": "ROUTINE", "chief_complaint": "Phase 1G verification - synthetic triage"},
        format="json",
    )
...
def full_content(abdominal=SYNTHETIC_ABDOMINAL, neurological=SYNTHETIC_NEUROLOGICAL):
    return {
        "presenting_complaint": "Phase 1G verification - synthetic presenting complaint",
        "hpi": "Phase 1G verification - synthetic HPI",
        "past_medical_history": "Phase 1G verification - synthetic past medical history",
        "past_surgical_history": "Phase 1G verification - synthetic past surgical history",
        "family_history": "Phase 1G verification - synthetic family history",
        "social_history": "Phase 1G verification - synthetic social history",
        "general_examination": SYNTHETIC_GENERAL,
        "abdominal_examination": abdominal,
        "neurological_examination": neurological,
        "consultation": "Phase 1G verification - synthetic assessment and plan note",
        "assessment": "Phase 1G verification - synthetic assessment",
        "plan": "Phase 1G verification - synthetic plan",
    }


def test_phase_1g_both_systems_round_trip_association_and_audit(tenant, authed_client):
...
@pytest.mark.parametrize("field_name", ["abdominal_examination", "neurological_examination"])
def test_phase_1g_exactly_2000_characters_are_accepted(tenant, authed_client, field_name):
    encounter = create_encounter(tenant, authed_client, label=f"Phase1GLimit{field_name}")
    value = "x" * 2000
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("neurological_examination", ["Phase 1G verification - invalid synthetic value"]),
    ],
)
def test_phase_1g_invalid_type_or_length_is_rejected_without_truncation(
    tenant, authed_client, field_name, invalid_value
):
    encounter = create_encounter(tenant, authed_client, label=f"Phase1GInvalid{field_name}")
    response = save_note(
        authed_client,
        encounter["id"],
        {field_name: invalid_value},
...
def test_phase_1g_partial_abdominal_and_neurological_saves_preserve_all_content(tenant, authed_client):
    encounter = create_encounter(tenant, authed_client, label="Phase1GPartial")
...
def test_phase_1g_other_field_update_preserves_both_system_examinations(tenant, authed_client):
    encounter = create_encounter(tenant, authed_client, label="Phase1GOtherField")
    initial = full_content()
    saved = save_note(authed_client, encounter["id"], initial, encounter["consultation_etag"])
    updated = save_note(
        authed_client,
        encounter["id"],
        {"hpi": "Phase 1G verification - updated synthetic HPI"},
        saved.data["etag"],
    )
...
def test_phase_1g_missing_if_match_fails_closed(tenant, authed_client):
    encounter = create_encounter(tenant, authed_client, label="Phase1GPrecondition")
...
def test_phase_1g_same_field_stale_write_is_rejected_without_overwrite(tenant, authed_client):
    encounter = create_encounter(tenant, authed_client, label="Phase1GSameField")
    baseline = save_note(
        authed_client,
        encounter["id"],
    ...
    conflict = save_note(
        authed_client,
        encounter["id"],
        {"abdominal_examination": "Phase 1G verification - stale synthetic abdominal examination"},
        baseline.data["etag"],
    )
...
def test_phase_1g_non_overlap_stale_retry_preserves_both_writers(tenant, authed_client):
    encounter = create_encounter(tenant, authed_client, label="Phase1GNonOverlap")
    initial = save_note(
        authed_client,
        encounter["id"],
    ...
    writer_a = save_note(
        authed_client,
        encounter["id"],
        {"general_examination": "Phase 1G verification - updated synthetic general examination"},
        initial.data["etag"],
    )
    stale = save_note(
        authed_client,
...
def test_phase_1g_sign_preserves_both_systems_and_version_content(tenant, authed_client):
    encounter = create_encounter(tenant, authed_client, label="Phase1GSign")
    initial = save_note(
        authed_client,
        encounter["id"],
        full_content(),
    ...
    rejected = save_note(
        authed_client,
        encounter["id"],
        {"abdominal_examination": "Phase 1G verification - rejected post-sign synthetic write"},
        signed.data["etag"],
    )
...
def test_phase_1g_stale_sign_preserves_draft_and_requires_fresh_retry(tenant, authed_client):
    encounter = create_encounter(tenant, authed_client, label="Phase1GStaleSign")
    initial = save_note(
        authed_client,
        encounter["id"],
...
def test_phase_1g_non_clinical_role_cannot_edit_system_examination(tenant, authed_client):
    encounter = create_encounter(tenant, authed_client, label="Phase1GRole")
    ...
    response = nurse_client.post(
        f"/api/v1/clinic/encounters/{encounter['id']}/notes/",
        {"content": {"neurological_examination": SYNTHETIC_NEUROLOGICAL}},
        format="json",
    )
...
def test_phase_1g_facility_and_tenant_isolation(tenant, authed_client):
    with tenant_atomic(tenant.organisation.id):
        other_facility = Facility.objects.create(
            organisation=tenant.organisation,
            name="Phase 1G Other Facility",
            code="OTHER",
            mode="CLINIC",
        )
    ...
    other_org = Organisation.objects.create(name="Other Phase 1G Clinic", slug="other-phase-1f")
